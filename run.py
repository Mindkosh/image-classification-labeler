...
class ClassificationLabeler:
	...
	def __init__(self, master):
		...
		if os.path.exists(self.recentActivityConfigFile):
			self.recentActivityConfig = open(self.recentActivityConfigFile).read().split(",")
		else:
		...
		self.prevImageButton = tk.Button(self.frameMIDDLEBUTTONS, text="<- Previous", command=self.prevImage, state=tk.DISABLED)
		self.prevImageButton.pack(side=tk.LEFT, padx=(0, 20))
		
		self.nextImageButton = tk.Button(self.frameMIDDLEBUTTONS, text="Next ->", command=self.nextImage, state=tk.DISABLED)
		...
		self.unlabeledImagesLabel = tk.Label(self.numberOfLabeledImages, text="0 unlabeled")
		...
		self.imageNumberLabel = tk.Label(self.frameMIDDLEIMGLABEL, text="Image:")
		...
		self.imgStage = tk.Canvas(self.frameMIDDLE, height=self.default_size[1], width=self.default_size[0], background="#292826")
		...
		self.imgStage.bind('<ButtonPress-1>', lambda event: self.imgStage.scan_mark(event.x, event.y))
		self.imgStage.bind("<B1-Motion>", lambda event: self.imgStage.scan_dragto(event.x, event.y, gain=1))
		...
		if self.imageSelection>0:
			if navigateLabeled:
				...
				if moveTo != -1:
					...
					self.statusBar.config(text="All images before this are labeled.")
		...
		if self.imageSelection<(len(self.imageListDir)-1):
			if navigateLabeled:
				...
				if moveTo != -1:
					...
					self.statusBar.config(text="All images after this are labeled.")
	...
	def loadImage(self, reset=False):
		...
		self.imgFileName.config(text=os.path.basename(str(self.imageListStr[self.imageSelection])))
		self.imageNumberInput.delete(0, tk.END)
		self.imageNumberInput.insert(0, str(self.imageSelection+1) + " / " + str(len(self.imageListDir)))
	...
	def saveImageCategorization(self):
		...
		self.unlabeledImagesLabel.config( text = str(self.unlabeledImages) + " unlabeled" )
	...
	def skipToImage(self, img_itr=None):
		try:
			if img_itr is not None:
				...
				tryImageSelection = int(self.imageNumberInput.get().split("/")[0].strip())-1
	...
	def loadDataSet(self, img_itr = None):
		...
		if self.dataSetSelection>=0:
			...
			self.unlabeledImages = len(self.imageListStr)
			self.imageLabels = []
			for imagei in range(len(self.imageListStr)):
				self.cursor.execute('SELECT labels.category_id FROM labels JOIN images ON labels.image_id = images.id WHERE images.imageName="'+str(self.imageListStr[imagei])+'"')
				labeled = False
				for category_id in self.cursor.fetchall():
					labeled = True
					self.unlabeledImages -= 1
					break
				self.imageLabels.append(labeled)
